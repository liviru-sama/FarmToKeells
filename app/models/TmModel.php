...
class TmModel {
    ...
    public function insertAdminCredentials($adminUsername, $hashedPassword, $email,$collectioncenter) {
        ...
        $this->db->query('INSERT INTO tm (admin_username, admin_password, email,collectioncenter) VALUES (:adminUsername, :hashedPassword, :email,:collectioncenter)');
...
{
    $this->db->query('SELECT * FROM tm WHERE admin_username = :adminUsername');
    $this->db->bind(':adminUsername', $adminUsername);
    $row = $this->db->single();
