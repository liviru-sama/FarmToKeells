...
<script>
    ...
    <a href="<?php echo URLROOT; ?>/admin/ccm_chat" style="text-decoration: none;">
                ...
                <h5 class="inline-heading" class
                = "tab-heading tab-selected" style="background: #65A534; transform: scale(1.08); border-radius: 10px 10px 10px 10px; padding: 10px;" >&nbsp;&nbsp;&nbsp;Chat with TM</h5>
            </a>
...
$prevSender = null;
$messageCount = count($data['tm_chats']);
...
?>
    ...
    <?php if ($isNewSender || $chat === end($data['tm_chats'])): ?>
        </div>
    <?php endif; ?>
...
<?php endforeach; ?>
        ...
        <form action="<?php echo URLROOT; ?>/admin/addChatadmintm" method="post">
           
                <label for="admin_reply"></label>
                <textarea name="admin_reply" rows="1" cols="50" required style="font-size:25px;" placeholder="Send message to TM..."></textarea>
