...
<h5 class="inline-heading"  class
=
"tab-heading tab-selected" style="background: #65A534; transform: scale(1.08); border-radius: 10px 10px 10px 10px; padding: 10px;">VIEW ALL USER ORDER</h5></a>

            <main class="table"></br>
                <section class="table_header">
                <h2>&nbsp;&nbsp;&nbsp;View all user orders </h2>
    ...
    <select class="statusInput" name="<?php echo is_array($row) ? 'status[]' : $row->status; ?>" onchange="submitForm(this)" <?php echo (is_array($row) && isset($row['status']) && ($row['status'] == 'Completed' || $row['status'] == 'Quality Rejected' || $row['status'] == 'Quality Approved')) ? 'style="pointer-events: none; pointer-events: none; opacity: 0.5; filter: grayscale(100%);"' : ($row['status'] == 'Pending Approval' ? 'style="background-color: rgba(255, 255, 255, 0.5); color: black;"' : 'style="background-color: red;opacity: 0.5; "'); ?>>
            ...
            <option style="background-color: red; pointer-events: none; opacity: 0.5; " value="Rejected" <?php echo (is_array($row) ? ($row['status'] == 'Rejected' ? 'selected' : '') : ($row->status == 'Rejected' ? 'selected' : '')); ?>>Rejected</option>
